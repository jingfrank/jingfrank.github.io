...
```text
┌──────────────────────────────────────────────────────────┐
│              传统 YOLO / Faster-RCNN 检测器               │
│  • 机制：闭集分类 (Closed-set Classification)            │
│  • 痛点：面对现实中形态千奇百怪的杂物 (薄膜/羽毛/棉纱)   │
│          由于未在训练集中标注，漏检率极高！              │
└────────────────────────────┬─────────────────────────────┘
                             │ (技术架构升级)
                             ▼
┌──────────────────────────────────────────────────────────┐
│              多模态视觉大模型 (Qwen-VL / VLM)             │
│  • 机制：开放词表零样本语义对齐 (Open-Vocabulary Grounding)│
│  • 优势：无需为每种异物采集数千张标注，用自然语言精准定位│
│  • 挑战：1. 4 帧多图 Token 爆炸；2. 单帧存在偶发静态幻觉  │
└──────────────────────────────────────────────────────────┘
```
...
```text
【4K 工业相机 RTSP 视频流 (25 FPS)】
                │
                ▼ (1 FPS 采样率，累计 4 帧时序窗口)
┌──────────────────────────────────────────────────────────┐
│  Stage 1：动态时序视频流全局分类 (640P / 4 帧连续送检)   │
│  • 任务：判定是否存在【大火花放电】或【镜头起雾脏污】    │
│  • 优势：图像缩放降低 75% Token，保留 4 秒时序动态过程   │
└────────────────────────────┬─────────────────────────────┘
                             │
                             ▼ (正常清晰工况下触发)
┌──────────────────────────────────────────────────────────┐
│  Stage 2：静态原图高保真单帧定位 (960P / 窗口最后一帧)   │
│  • 任务：原图高分辨率像素定位滑板上的【悬挂异物坐标】    │
│  • 输出：XML 格式中心点坐标 <point>x, y</point>          │
│  • 优势：避免多帧插值造成的微小羽毛/细线模糊             │
└────────────────────────────┬─────────────────────────────┘
                             │
                             ▼
┌──────────────────────────────────────────────────────────┐
│  抗虚警网关：跨窗口时序一致性追踪器 (Temporal Tracker)   │
│  • 连续 2 次窗口在同一空间坐标命中才确诊告警             │
└──────────────────────────────────────────────────────────┘
```
...
我们提出 **`TemporalConsistencyTracker`（空间-时序一致性滤波追踪器）**：
